        ...
        if "signal" in data_dict:
            for mod_name, mod_info in data_dict["signal"].items():
                ...
                if "data" in mod_info:
                    ...
                    if data.ndim != 2:
                        errors.append(
                            f"信号模态 '{mod_name}' 的 data 必须是2维数组 (channels x time)。当前维度: {data.ndim}")
                    elif data.shape[0] != len(ch_names):
                        errors.append(
                            f"信号模态 '{mod_name}' 的通道数 ({data.shape[0]}) 与 channel_names 长度 ({len(ch_names)}) 不匹配。")
...
class BioSignalReader:
    ...
    def _read_snirf_wrapper(self, file_path, **kwargs):
        ...
        raw_data = {
            ...
            'sampling_rate': 1.0 / np.mean(
                np.diff(data_from_reader.get('time', [0, 1]))) if 'time' in data_from_reader else 10.0,
            'channel_names': data_from_reader.get('channel_names', []),
            'unit': 'optical_density',
